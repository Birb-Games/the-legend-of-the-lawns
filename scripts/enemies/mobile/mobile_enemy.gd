...
func update_path() -> void:
	var player_tile_pos = Vector2i(
		floor(player.global_position.x / lawn.tile_size.x),
		floor(player.global_position.y / lawn.tile_size.y)
	)
	current_player_tile_pos = player_tile_pos
	path = lawn.astar_grid.get_point_path(get_tile_pos(), player_tile_pos)
...
func _process(delta: float) -> void:
	...
	if player_tile_pos != current_player_tile_pos:
